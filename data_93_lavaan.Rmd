...
Impute data (currently not included)
```{r}
# #install.packages("NPBayesImputeCat")
# library(NPBayesImputeCat)
# model <- CreateModel(data[,5:ncol(data)], MCZ = NULL, 10,10000,0.25,0.25,17)
# model$Run(20,40,2)
...
### lavaan model 1
```{r}
varname_incl <- c("DRINKCAT","B17_A","B17_E","B17_G","B17_H","B17_I","B17_J",
                  "B17_K","B17_B","B17_C","B17_D","B17_F","AGEGROUP","SEX","AGEGROUP",
                  "GRADE","OFFCAMP","SECALC_A","SECALC_B","SECALC_C","SECALC_D",
                  "SECALC_E","SECALC_G","SECALC_H","SECALC_I")
dat93_lavmodel <- dat93_lavmodel[,names(dat93_lavmodel) %in% varname_incl]
dat93_lavmodel <- na.omit(dat93_lavmodel)
HS.model <- '
      # latent variables
        personal =~ B17_A + B17_E + B17_G + B17_H + B17_I + B17_J + B17_K
        communal =~ B17_B + B17_C + B17_D + B17_F
      # regressions
        personal ~ DRINKCAT + SEX + AGEGROUP + GRADE
        communal ~ DRINKCAT + SEX + AGEGROUP + GRADE +
                    SECALC_A + SECALC_B + SECALC_C + SECALC_D + 
                    SECALC_E + SECALC_G + SECALC_H + SECALC_I + OFFCAMP
      '
...
HS.model <- '
      ...
      # regressions
        ...
        SECALC_C + SECALC_D + SECALC_E + SECALC_G + SECALC_H + SECALC_I + OFFCAMP)
      '
fit <- cfa(HS.model, data = dat93_lavmodel, ordered=c("DRINKCAT",
                                                      "B17_A","B17_E","B17_G","B17_H","B17_I",
                                                      "B17_J","B17_K","B17_B","B17_C","B17_D",
...
#       '
# 
# fit <- blavaan(HS.model, data=dat93_lavmodel,ordered=c("DRINKCAT",
#                                                       "B17_A","B17_E","B17_G","B17_H","B17_I",
#                                                       "B17_J","B17_K","B17_B","B17_C","B17_D",
#                                                       "B17_F","AGEGROUP"))
```
Error in blavaan(HS.model, data = dat93_lavmodel, ordered = c("DRINKCAT", : blavaan ERROR: models with ordered variables are not yet available.
